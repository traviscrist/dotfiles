...
- `gpt-6-astra` is allowlisted for `openai` and `openai-codex`. When explicitly enabled, Fast Mode requests `service_tier: "priority"`; the Fast status indicates a request, not verified provider acceptance or a guaranteed speedup.
