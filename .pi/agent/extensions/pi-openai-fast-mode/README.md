...
- `defaults.json` is read-only configuration; Fast Mode defaults to enabled (priority service).
...
- `gpt-6-astra` is allowlisted for `openai` and `openai-codex`. Fast Mode requests `service_tier: "priority"` by default; the Fast status indicates a request, not verified provider acceptance or a guaranteed speedup.
